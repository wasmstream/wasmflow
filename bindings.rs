pub mod record_processor {
  ...
  impl<'a,> std::fmt::Debug for FlowRecord<'a,> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
      f.debug_struct("FlowRecord").field("key", &self.key).field("value", &self.value).field("headers", &self.headers).field("offset", &self.offset).finish()}
  }
  ...
  pub struct RecordProcessor<T> {
    ...
    parse_record: wasmtime::TypedFunc<(i32,i32,i32,i32,i32,i32,i32,i32,i64,), (i32,)>,
  }
  impl<T> RecordProcessor<T> {
    ...
    get_state: impl Fn(&mut T) -> &mut RecordProcessorData + Send + Sync + Copy + 'static,
      ...
      ;
      let parse_record= instance.get_typed_func::<(i32,i32,i32,i32,i32,i32,i32,i32,i64,), (i32,), _>(&mut store, "parse-record")?;
      Ok(RecordProcessor{
    ...
    pub fn parse_record(&self, mut caller: impl wasmtime::AsContextMut<Data = T>,rec: FlowRecord<'_,>,)-> Result<Status, wasmtime::Trap> {
      ...
      for (i, e) in vec8.into_iter().enumerate() {
        let base = result8 + (i as i32) * 16;
        {
          let (t5_0, t5_1, ) = e;
          let vec6 = t5_0;
          let ptr6 = func_canonical_abi_realloc.call(&mut caller, (0, 0, 1, vec6.len() as i32))?;
        ...
        }}let (result9_0,) = self.parse_record.call(&mut caller, (result2_0, result2_1, result2_2, result4_0, result4_1, result4_2, result8, len8, wit_bindgen_wasmtime::rt::as_i64(offset0), ))?;
        Ok(match result9_0 {
          0 => Status::Ok,
          1 => Status::Error,
